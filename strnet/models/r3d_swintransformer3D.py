...
from ..blocks.r3d_block import Uper, Down, Out
...
class BasicLayer(nn.Module):
    ...
    Args:
        ...
        num_heads (int): Number of attention head of each stage.
...
class PatchEmbed3D(nn.Module):
    ...
    Args:
        ...
        temp_stride (int): Temporal stride for patch embedding. Default: 1
    """
    ...
    def forward(self, x):
        ...
        x = self.proj(x)  # B C D/temp_stride Wh Ww
        # print(x.size())
...
class MFEblock3D(nn.Module):
    def __init__(self, in_channels, atrous_rates):
        super(MFEblock3D, self).__init__()
        ...
        self.layer1 = nn.Sequential(
            ...
            nn.BatchNorm3d(out_channels),
            nn.ReLU())
        self.layer2 = ASPPConv3D(in_channels, out_channels, rate1)
        self.layer3 = ASPPConv3D(in_channels, out_channels, rate2)
        self.layer4 = ASPPConv3D(in_channels, out_channels, rate3)
        ...
        self.SE1 = oneConv3D(in_channels, in_channels, 1, 0, 1)
        self.SE2 = oneConv3D(in_channels, in_channels, 1, 0, 1)
        self.SE3 = oneConv3D(in_channels, in_channels, 1, 0, 1)
        self.SE4 = oneConv3D(in_channels, in_channels, 1, 0, 1)
...
class SwinTransformer3D(nn.Module):
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.layers):
            if i ==0 :
                x1 = layer(x0.contiguous())
                x1_out = self.proj_out(x1)
            if i ==1 :
                x2 = layer(x1.contiguous())
                x2_out = self.proj_out(x2)
            if i ==2 :
                x3 = layer(x2.contiguous())
                x3_out = self.proj_out(x3)
            if i ==3 :
                x4 = layer(x3.contiguous())
